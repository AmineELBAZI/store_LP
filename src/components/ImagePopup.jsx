...
const ImagePopup = () => {
  ...
  return (
    <div className="w-full bg-[#eaf1f5] p-0 items-center">
      <h2 className="text-3xl md:text-4xl font-bold text-white  min-w-8 w-1/3 rounded-r-full rounded-tr-none bg-cyan-600 p-6  mb-8 hover:bg-white hover:text-cyan-600">
        Nos Offres d'Emploi :
      </h2>
      <div className=" w-full bg-[#eaf1f5] mx-auto py-12 items-center ">
      <Slider {...carouselSettings}>
        {carouselImages.flatMap((item, index) => (
          <div key={index} className=" items-center">
            <div
              className={`flex items-center bg-white border rounded-lg shadow-xl ${index === index + 1 ? "transform scale-110 z-10 p-2 rounded-l-lg" : "transform scale-90"}`}
              style={{
                width: "100%",
                maxWidth: "1300px",
            ...
            >
              ...
              {/* Content Column */}
              <div className="w-full md:w-1/2 h-auto p-4 flex flex-col justify-between">
                <h5 className="mb-2 text-xl md:text-3xl font-bold text-neutral-800">{item.title}</h5>
                <p className="mb-3 text-base md:text-lg text-gray-700 p-8">{item.description}</p>
                <button
                  className="bg-blue-500 text-white px-4 py-2 rounded-tr-full hover:bg-blue-600 text-lg md:text-2xl"
        ...
        >
          ...
          <div className="h-full overflow-y-auto">
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-3xl font-bold text-neutral-800 m-2">{modalContent.title}</h2>
              <div className="flex items-center p-4 m-4 border-2 border-solid border-blue-300 rounded-lg">
                ...
                <b className="text-gray-900 m-6 ml-8 text-2xl mb-1">
                  {modalContent.Pays} : <span className="text-blue-600">{modalContent.Ville}</span>
                </b>
              </div>
            </div>
            <p className="text-gray-700 m-6 ml-8 text-2xl mb-1">{modalContent.description}</p>
            <p className="text-600 m-6 ml-8 text-2xl text-neutral-800 mb-1">Compétences requises</p>
            ...
            <button
              ...
              onClick={() => {
                const queryParam = new URLSearchParams({ Object: modalContent.title }).toString();
